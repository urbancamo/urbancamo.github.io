...
3. String expressions are displayed unchanged. There are, however, special
...
display (`STOP` appears on display). Pressing `EXE` executes a carriage
return/line feed and proceeds to the next display.
...
Pressing `EXE` executes a line change and proceeds to the next display.
