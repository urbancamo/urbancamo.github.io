...
1. Reads from the file specified by the file descriptor to the currently specified
   program area. The format of the file can be either internal or ASCII format.
...
9. The first file on the cassette tape with an attribute which matches the one specified is the default option when the entire file descriptor is omitted.
