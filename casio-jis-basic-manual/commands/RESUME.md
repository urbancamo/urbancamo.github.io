...
### Format
```basic
RESUME [ NEXT | return line number ]
```
...
5. A RESUME statement without a return destination or a RESUME statement
that specifies the line in which the original error was generated as the return line
number cannot be written at the beginning of the error handling routine.
...
6. A RESUME statement must always be included in the same program area
