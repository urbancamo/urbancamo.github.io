...
Specifies a [DATA](DATA.md) line for reading by the [READ](READ.md) statement.
...
1. The first [DATA](DATA.md) line in the program file containing the [READ](READ.md)
...
[DATA](DATA.md) line is read by the next [READ](READ.md) statement execution.
A UL error is generated when the specified line number
does not exist, while a DA error is generated when no
