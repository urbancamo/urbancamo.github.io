...
or the program jumps to the specified branch destination. Execution proceeds to the
next program line when the `ELSE` statement is omitted.
3. The format `IF A THEN ~` results in the condition being met when value of the expression (A) is not 0 (absolute value of A < 1 x 10^-99). The condition is not met when the value of the expression is 0.
